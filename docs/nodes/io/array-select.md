...
*[PLACEHOLDER - Add GIF showing various selection patterns on arrays with visual results]*
...
```
[Array-Out] → [Array-Select] → [Transform] → [Save Results]
```
Select specific elements for further processing.

### Multi-Path Selection
```
[Array] → [Array-Select: "0,2,4"] → [Process Even]
       → [Array-Select: "1,3,5"] → [Process Odd]
```
Split array into multiple processing paths.

### Conditional Selection
```
[Array] → [Switch] → [Array-Select: msg.pattern] → [Conditional Output]
```
Dynamic selection based on conditions.

### Result Aggregation
```
[Array-Select: "0"] → [Best Result]
[Array-Select: "-1"] → [Last Result]  
[Array-Select: "::5"] → [Sample Results]
```
Multiple selection patterns from same array.

## Advanced Usage

### Dynamic Selection Patterns
```javascript
// In a function node before array-select
msg.selectionPattern = msg.testMode ? "0::2" : "1::2";
```
Change selection based on runtime conditions.

### Nested Array Handling
- **Pattern**: Use multiple array-select nodes for nested structures
- **Example**: First select outer array elements, then select from inner arrays

### Performance Optimization
- **Large Arrays**: Use ranges instead of comma-separated indices
- **Frequent Selection**: Cache selection patterns in flow/global context
- **Memory Efficiency**: Consider processing selected elements immediately

### Error Recovery
```
[Array-Select] → [Switch: array.length > 0] → [Process]
                                           → [Handle Empty]
```
Handle empty selection results gracefully.

## Best Practices

### Selection Pattern Design
- Use ranges for contiguous selections: `1:5` vs `1,2,3,4`
- Use step patterns for regular sampling: `::5` vs `0,5,10,15,...`
- Use negative indices for end-relative selection: `-3:` vs calculating positions

### Error Handling
- Always validate array input before selection
- Handle empty selection results in downstream nodes
- Use debug nodes to verify selection patterns

### Performance
- Prefer simple patterns over complex comma-separated lists
- Use Force Array when downstream nodes expect arrays
- Consider memory usage with large arrays and complex selections